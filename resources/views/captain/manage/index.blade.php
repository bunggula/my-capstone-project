...
<div class="flex h-screen bg-gray-100">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="sticky top-0 z-10 bg-white shadow">
            @include('partials.captain-header', ['title' => '📄 Document Requests'])
        </div>
                                ...
                                <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2 items-center">
                                            ...
                                            <!-- Hidden delete form -->
                                            <form id="delete-form-{{ $document->id }}" action="{{ route('captain.manage.destroy', $document->id) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
...
<div id="addModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-xl p-6">
        ...
        <form action="{{ route('captain.manage.store') }}" method="POST">
            @csrf
            <div class="mb-4">
...
function fetchDocument(id) {
    fetch("{{ url('/captain/manage') }}/" + id + "/edit")
        .then(res => res.json())
        .then(data => {
            document.getElementById('editForm').action = "{{ url('/captain/manage') }}/" + id;
