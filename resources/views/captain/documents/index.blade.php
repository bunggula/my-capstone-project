...
<div class="flex font-sans min-h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col bg-gray-100 h-screen overflow-hidden">
        ...
        <div class="sticky top-0 z-10 bg-white shadow">
            @include('partials.captain-header', ['title' => '📄 Document'])
        </div>
    ...
    <a href="{{ route('captain.documents.index', ['document_type' => $docTypeFilter]) }}"
...
<form method="GET" action="{{ route('captain.documents.index') }}" class="flex flex-wrap items-center gap-4 mb-4">
...
<select name="document_type" onchange="this.form.submit()" 
    ...
    @foreach($documentTypes as $type)
        <option value="{{ $type }}" {{ $docTypeFilter == $type ? 'selected' : '' }}>{{ $type }}</option>
    @endforeach
</select>
...
{{-- 🧹 Clear Filters --}}
@if(request('search') || request('document_type') || request('month') || request('year'))
...
<table class="w-full table-auto border text-sm">
    ...
    <tbody class="bg-white divide-y divide-gray-100">
        ...
        <tr class="{{ $loop->odd ? 'bg-white' : 'bg-gray-100' }} border-t">
            <td class="px-4 py-3">{{ $requests->firstItem() + $loop->index }}</td>
            <td class="px-4 py-3">{{ $request->reference_code ?? 'N/A' }}</td>
            <td class="px-4 py-3">{{ $request->resident->full_name ?? 'N/A' }}</td>
<td class="px-4 py-3">
    ...
    @endif
           ...
           <td class="px-4 py-3">
    {{ $request->created_at ? \Carbon\Carbon::parse($request->created_at)->format('F d, Y | g:i A') : 'N/A' }}
</td>
