        ...
        <div class="block p-4 rounded shadow border-l-4 bg-gray-100 border-gray-400 text-gray-700 w-full sm:w-1/3">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $totalBlotters }}</p>
                </div>
...
<div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 w-full">
    ...
    <form id="filterForm" action="{{ route('secretary.blotter.index') }}" method="GET"
          class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 w-full">

        {{-- Search --}}
        <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="Search blotter..."
                        ...
                        <tbody>
                            @foreach($blotters as $b)
                                <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200">
                                    {{-- Date --}}
                                     <td class="px-4 py-2">
                                        {{ $blotters->firstItem() + $loop->index }}
                                    </td>
                                    <td class="px-4 py-2">
                                        {{ \Carbon\Carbon::parse($b->date)->format('F j, Y') }}
                                    </td>

                                    {{-- Time --}}
                                    <td class="px-4 py-2">
                                        {{ \Carbon\Carbon::parse($b->time)->format('g:i a') }}
                                    </td>

                                    {{-- Complainants --}}
                                    <td class="px-4 py-2">
                                        <ul class="ml-2 space-y-1 list-none">
                                            @foreach($b->complainants as $c)
...
@endif
                                    ...
                                    <td class="px-4 py-2 text-center space-x-2 flex justify-center">
                                        ...
                                        {{-- Print --}}
                                        <a href="{{ route('secretary.blotter.print', $b->id) }}" target="_blank"
                                        ...
                                        {{-- Upload Signed --}}
@if(!$b->is_signed)
    <button onclick="openUploadModal({{ $b->id }})"
...
<div id="addBlotterModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50 p-4">
    <div class="bg-white w-full max-w-3xl rounded-lg shadow-lg p-6 relative overflow-y-auto max-h-[90vh]">
        ...
        <form action="{{ route('secretary.blotter.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="date" class="border p-2 w-full rounded" required max="{{ date('Y-m-d') }}">
                </div>
                <div>
...
{{-- ================= JS ================= --}}
<script>
    let residents = @json($residents); // Laravel JSON of residents
