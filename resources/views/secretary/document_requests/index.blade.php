...
<div class="flex font-sans min-h-screen overflow-hidden">
   ...
   <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
    {{-- Pending --}}
    <a href="{{ route('secretary.document_requests.index', array_merge(request()->all(), ['status' => 'pending'])) }}"
    ...
    {{-- Approved --}}
    <a href="{{ route('secretary.document_requests.index', array_merge(request()->all(), ['status' => 'approved'])) }}"
    ...
    {{-- Rejected --}}
    <a href="{{ route('secretary.document_requests.index', array_merge(request()->all(), ['status' => 'rejected'])) }}"
    ...
    {{-- Ready for Pickup --}}
    <a href="{{ route('secretary.document_requests.index', array_merge(request()->all(), ['status' => 'ready_for_pickup'])) }}"
       class="p-4 rounded shadow border-l-4 text-sm {{ $currentStatus === 'ready_for_pickup' ? 'bg-purple-200 border-purple-600 text-purple-900' : 'bg-purple-100 border-purple-500 text-purple-700' }}">
        <div class="flex items-center justify-between">
            <div>
                <h4 class="font-semibold">Ready for Pickup</h4>
                <p class="text-xl font-bold">{{ $counts['ready_for_pickup'] ?? 0 }}</p>
            </div>
            <svg class="h-6 w-6 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    {{-- Completed --}}
    <a href="{{ route('secretary.document_requests.index', array_merge(request()->all(), ['status' => 'completed'])) }}"
       class="p-4 rounded shadow border-l-4 text-sm {{ $currentStatus === 'completed' ? 'bg-blue-200 border-blue-600 text-blue-900' : 'bg-blue-100 border-blue-500 text-blue-700' }}">
        <div class="flex items-center justify-between">
            <div>
                <h4 class="font-semibold">Completed</h4>
                <p class="text-xl font-bold">{{ $counts['completed'] ?? 0 }}</p>
            </div>
            <svg class="h-6 w-6 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<form method="GET" action="{{ route('secretary.document_requests.index') }}"
      id="autoSearchForm"
...
<tbody class="divide-y divide-gray-100">
    ...
    <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200">
        <td class="px-4 py-2">{{ $requests->firstItem() + $loop->index }}</td>
    ...
    {{ $typeLabel }}
        ...
        <td class="px-4 py-2">{{ \Carbon\Carbon::parse($request->created_at)->format('F d, Y | g:i A') }}</td>
        <td class="px-4 py-2">
            <span class="px-2 py-1 rounded text-xs font-medium
                ...
                @elseif ($request->status === 'ready_for_pickup') bg-purple-100 text-purple-700
                @elseif ($request->status === 'completed') bg-blue-100 text-blue-700
                @endif">
                {{ ucfirst(str_replace('_', ' ', $request->status)) }}
                               ...
                               {{-- 👁 View Details --}}
                               <button type="button"
    onclick="showRequestDetails({{ $request->toJson() }})"
...
@if ($request->status === 'pending')
    {{-- ✅ Approve --}}
    <form method="POST" action="{{ route('secretary.document_requests.update_status', ['id' => $request->id, 'status' => 'approved']) }}" class="approve-form">
    @csrf
    <button type="button"
    onclick="confirmApprove(this)"
    data-resident="{{ $request->resident->full_name }}"
    data-document="{{ $request->document_type }}"
    data-is-custom="{{ $request->is_custom_purpose ? 'true' : 'false' }}"
    ...
    {{-- ❌ Reject --}}
    <form method="POST" action="{{ route('secretary.document_requests.update_status', ['id' => $request->id, 'status' => 'rejected']) }}" class="reject-form">
    @csrf
    <button type="button"
        onclick="confirmReject(this, '{{ $request->resident->full_name }}', '{{ $request->document_type }}')"
        title="Reject"
    ...
    {{-- 🖨️ Print --}}
      <a href="{{ route('secretary.document_requests.print', $request->id) }}"
        ...
        {{-- 📦 Ready for Pickup --}}
        <form method="POST" action="{{ route('secretary.document_requests.update_status', ['id' => $request->id, 'status' => 'ready_for_pickup']) }}" class="pickup-form">
    @csrf
    <button type="button"
        onclick="confirmReadyForPickup(this, '{{ $request->resident->full_name }}', '{{ $request->document_type }}')"
    ...
    {{-- 🖨️ Print --}}
    <a href="{{ route('secretary.document_requests.print', $request->id) }}" title="Print" target="_blank"
        class="bg-green-600 hover:bg-green-700 text-white p-2 rounded transition">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2M6 14h12v8H6v-8z" />
        ...
        {{-- ✅ Complete --}}
        <form method="POST" action="{{ route('secretary.document_requests.mark_completed', $request->id) }}" class="complete-form">
    @csrf
    <button type="button"
        onclick="confirmComplete(this, '{{ $request->resident->full_name }}', '{{ $request->document_type }}')"
