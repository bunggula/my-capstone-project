...
x-data="{ 
    ...
    <div class="flex flex-col flex-1 max-h-screen">
        @include('partials.secretary-header', ['title' => 'Events and Announcements'])
...
<h1 class="text-2xl font-bold text-blue-900 mb-6">
    Events  - Barangay {{ Auth::user()->barangay->name ?? 'N/A' }}
</h1>

{{-- Filter Cards + Add Button --}}
<div class="space-y-4 mb-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full">
        {{-- Pending --}}
       {{-- Pending --}}
<a href="{{ route('secretary.events.index', array_merge(request()->except('status'), ['status' => 'pending'])) }}"
...
{{-- Approved --}}
<a href="{{ route('secretary.events.index', array_merge(request()->except('status'), ['status' => 'approved'])) }}"
...
{{-- Rejected --}}
<a href="{{ route('secretary.events.index', array_merge(request()->except('status'), ['status' => 'rejected'])) }}"
...
{{-- All --}}
<a href="{{ route('secretary.events.index', array_merge(request()->except('status'), ['status' => 'all'])) }}"
...
{{-- Search & Filter Form --}}
<form method="GET" action="{{ route('secretary.events.index') }}" class="w-full md:w-auto" id="eventSearchForm">
...
<script>
    ...
    @elseif ($event->status === 'pending') bg-yellow-100 text-yellow-700
    ...
    <div class="flex justify-center items-center space-x-1">
        {{-- View --}}
        <button 
            @click.prevent="selectedEvent = JSON.parse(atob('{{ base64_encode(json_encode($event->load('images'))) }}')); viewModal = true"
        ...
        <button 
            @click.prevent="selectedEvent = JSON.parse(atob('{{ base64_encode(json_encode($event->load('images'))) }}')); editModal = true"
...
<form 
    action="{{ route('secretary.events.resubmit', $event->id) }}" 
    method="POST" 
    class="inline resubmit-form"   {{-- add this class --}}
    data-title="{{ $event->title }}" {{-- add this attribute --}}
>
    @csrf
    @method('PATCH')
    <button type="submit"
...
@endif
      ...
      {{-- Delete --}}
<form action="{{ route('secretary.events.destroy', $event->id) }}" method="POST" class="inline delete-form" data-title="{{ $event->title }}">
    @csrf
    @method('DELETE')
    <button type="submit"
...
>
    ...
    >
        @include('secretary.events.create') {{-- ✔ form content --}}
    </div>
</div>
