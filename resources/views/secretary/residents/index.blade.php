...
<div class="flex flex-wrap justify-between items-center mb-4">
    <h2 class="text-2xl font-bold text-blue-900 mb-6">
        Residents of Barangay - {{ Auth::user()->barangay->name ?? 'N/A' }}
        @if (session('success'))
        <script>
...
<div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
    {{-- Approved --}}
    <a href="{{ route('secretary.residents.index', array_merge(request()->query(), ['status' => 'approved', 'gender' => null])) }}"
    ...
    {{-- Archived --}}
    <a href="{{ route('secretary.residents.index', array_merge(request()->query(), ['status' => 'archived', 'gender' => null])) }}"
    ...
    {{-- Male --}}
    <a href="{{ route('secretary.residents.index', array_merge(request()->query(), ['status' => 'approved', 'gender' => 'Male'])) }}"
    ...
    {{-- Female --}}
    <a href="{{ route('secretary.residents.index', array_merge(request()->query(), ['status' => 'approved', 'gender' => 'Female'])) }}"
...
<form method="GET" action="{{ route('secretary.residents.index') }}" class="flex flex-wrap items-center justify-between gap-4 mb-6" id="residentSearchForm">
...
document.getElementById('combinedFilter').addEventListener('change', function() {
    ...
    <a href="{{ route('secretary.residents.print', [
                ...
                <div class="overflow-x-auto">
                    <table class="w-full table-auto border text-sm">
                        ...
                        <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200">
                             <td class="px-4 py-2 ">{{ $residents->firstItem() + $loop->index }}</td>
    ...
    {{ $resident->first_name }} {{ $resident->middle_name }} {{ $resident->last_name }} {{ $resident->suffix }}
                                ...
                                <td class="px-4 py-2 ">{{ \Carbon\Carbon::parse($resident->birthdate)->format('M d, Y') }}</td>
                                <td class="px-4 py-2 ">{{ $resident->civil_status }}</td>
<td class="px-4 py-2 text-center space-x-2">

    {{-- ✅ View --}}
    <button onclick='openViewModal(@json($resident))'
    ...
    {{-- ✅ Edit --}}
    <button onclick='openEditModal(@json($resident))'
    ...
    {{-- ✅ Generate Document (walk-in only) --}}
    @if($resident->type === 'walk-in')
    <button type="button"
        class="generate-document-btn bg-green-600 hover:bg-green-700 text-white p-2 rounded-md transition"
        data-resident='@json($resident, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP)'
    ...
    <form method="POST"
        ...
        <button type="button"
            ...
            class="{{ $resident->status === 'archived' 
                        ? 'bg-green-600 hover:bg-green-700' 
                        : 'bg-yellow-600 hover:bg-yellow-700' }}
                   text-white p-2 rounded-md transition archive-button"
            title="{{ $resident->status === 'archived' ? 'Unarchive' : 'Archive' }}">
...
<div id="documentModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-lg relative">
        ...
        <!-- Close Button -->
        <button id="closeDocumentModal" type="button" 
        ...
        <form id="documentRequestForm" method="POST" action="{{ route('secretary.documents.request') }}">
            @csrf
            ...
            <select name="purpose_id" id="purpose_id" class="border rounded px-3 py-2 w-full mb-4" required>
                ...
                @foreach($documents as $doc)
                    <option value="{{ $doc->id }}">{{ $doc->purpose }}</option>
                @endforeach
...
{{-- ✅ Create Modal --}}
@include('secretary.residents.create')

{{-- ✅ Edit Modal --}}
@include('secretary.residents.edit')
