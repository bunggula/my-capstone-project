...
<div class="flex h-screen overflow-y-auto bg-gray-100 text-gray-800">
   ...
   <div class="flex-1 flex flex-col overflow-hidden">
     ...
     <div class="flex-1 overflow-y-auto p-6">
            ...
            <form method="GET" action="{{ route('secretary.reports.services') }}" 
    ...
    <select name="document_type" id="document_type" 
        ...
        @foreach($documentTypes as $type)
            <option value="{{ $type }}" {{ ($documentType ?? '') == $type ? 'selected' : '' }}>
                {{ $type }}
            </option>
        @endforeach
    </select>
...
<div id="weekly_inputs" class="hidden flex gap-2">
    ...
    @if(request()->hasAny(['document_type','date_filter','daily_date','start_date','end_date','month']) 
        && collect(request()->only(['document_type','date_filter','daily_date','start_date','end_date','month']))->filter()->isNotEmpty())
        <a href="{{ route('secretary.reports.services') }}"
...
@if(request()->hasAny(['document_type','date_filter','daily_date','start_date','end_date','month']) 
    && collect(request()->only(['document_type','date_filter','daily_date','start_date','end_date','month']))->filter()->isNotEmpty())
    <a href="{{ route('secretary.reports.services.print', [
    ...
    @foreach($completedRequests as $index => $request)
      <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200">
            <td class="border px-3 py-2 text-center">
                {{ $completedRequests->firstItem() ? $completedRequests->firstItem() + $loop->index : $loop->iteration }}
            </td>
            <td class="border px-3 py-2">{{ $request->resident->fullname ?? 'N/A' }}</td>
            <td class="border px-3 py-2">{{ ucfirst(str_replace('_', ' ', $request->document_type)) }}</td>
            <td class="border px-3 py-2">{{ $request->purpose ?? 'N/A' }}</td>
            <td class="border px-3 py-2 text-right">
                {{ $request->price ? '₱' . number_format($request->price, 2) : 'N/A' }}
            </td>
            <td class="border px-3 py-2">
                {{ $request->completed_at ? \Carbon\Carbon::parse($request->completed_at)->format('F d, Y') : '-' }}
            </td>
                    </tr>
                @endforeach
            </tbody>
