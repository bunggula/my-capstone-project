...
<div class="flex h-screen bg-gray-100 overflow-hidden">
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        @include('partials.secretary-header', ['title' => 'Residents'])
...
<div @click.away="showModal = false"
    ...
    <!-- Modal Content -->
    <div class="space-y-6">
        @include('secretary.officials.create')
    </div>
</div>
</div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 gap-y-6 text-gray-700 text-sm sm:text-base">
        ...
        <div class="p-4 bg-gray-50 rounded-lg shadow-sm">
            ...
            <p class="mt-1 text-gray-900 font-medium">
                {{ $captain->first_name }} {{ $captain->middle_name }} {{ $captain->last_name }}{{ $captain->suffix ? ' ' . $captain->suffix : '' }}
            </p>
        </div>
...
<div @click.away="showViewSecretary = false"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 gap-y-6 text-gray-700 text-sm sm:text-base">
        ...
        <div class="p-4 bg-gray-50 rounded-lg shadow-sm">
            ...
            <p class="mt-1 text-gray-900 font-medium">
                {{ $secretary->first_name }} {{ $secretary->middle_name }} {{ $secretary->last_name }}{{ $secretary->suffix ? ' ' . $secretary->suffix : '' }}
            </p>
        </div>
        ...
        {{-- Other Officials / SK Chairman / Barangay Officials --}}
        @forelse ($officials as $official)
            <tr x-data="{ showView{{ $official->id }}: false, showEdit{{ $official->id }}: false }" class="bg-white">
                <td class="border px-4 py-2">
                    {{ $official->last_name }}, {{ $official->first_name }} {{ $official->middle_name }}{{ $official->suffix ? ' ' . $official->suffix : '' }}
                </td>
...
<!-- Edit Icon Button -->
<button @click="showEdit{{ $official->id }} = true"
    ...
    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
       ...
       <!-- View Modal -->
<div x-show="showView{{ $official->id }}" 
 x-transition.opacity
 x-cloak
 class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 py-12 px-4">

<div @click.away="showView{{ $official->id }} = false"
     class="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl overflow-y-auto max-h-[90vh] transform transition-all scale-95 duration-200 ease-out"
     x-transition:enter="transition ease-out duration-200"
     x-transition:enter-start="opacity-0 scale-95"
     x-transition:enter-end="opacity-100 scale-100"
    ...
    <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-3">
        ...
        <button @click="showView{{ $official->id }} = false"
                class="text-gray-400 hover:text-red-600 text-2xl font-bold transition-colors">&times;</button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 gap-y-6 text-gray-700 text-sm sm:text-base">
        ...
        <div class="p-4 bg-gray-50 rounded-lg shadow-sm">
            <p class="text-gray-500 font-semibold uppercase text-xs">Full Name</p>
            <p class="mt-1 text-gray-900 font-medium">
                {{ $official->first_name }} {{ $official->middle_name }} {{ $official->last_name }}{{ $official->suffix ? ' ' . $official->suffix : '' }}
            </p>
        </div>
        ...
        <div class="p-4 bg-gray-50 rounded-lg shadow-sm">
            <p class="text-gray-500 font-semibold uppercase text-xs">Age</p>
            <p class="mt-1 text-gray-900 font-medium">{{ $official->age }}</p>
        </div>

        <div class="p-4 bg-gray-50 rounded-lg shadow-sm">
            <p class="text-gray-500 font-semibold uppercase text-xs">Status</p>
            <p class="mt-1 text-gray-900 font-medium">{{ $official->civil_status ?? '-' }}</p>
        </div>

        <div class="p-4 bg-gray-50 rounded-lg shadow-sm">
            <p class="text-gray-500 font-semibold uppercase text-xs">Phone Number</p>
            <p class="mt-1 text-gray-900 font-medium">{{ $official->phone ?? '-' }}</p>
        </div>
...
<!-- Edit Official Modal -->
<div x-cloak x-show="showEdit{{ $official->id }}" 
 class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
<div @click.away="showEdit{{ $official->id }} = false"
    ...
    <div class="flex justify-between items-center border-b border-gray-200 pb-4 mb-6">
        ...
        <button @click="showEdit{{ $official->id }} = false" 
                class="text-gray-400 hover:text-red-600 text-4xl font-bold transition-colors">&times;</button>
    </div>

    <!-- Form -->
    <form action="{{ route('secretary.officials.update', $official->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <!-- Name Fields -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <input type="text" name="first_name" value="{{ $official->first_name }}" placeholder="First Name" required
                   class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
            <input type="text" name="middle_name" value="{{ $official->middle_name ?? '' }}" placeholder="Middle Name"
                   class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
            <input type="text" name="last_name" value="{{ $official->last_name }}" placeholder="Last Name" required
                   class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
            <input type="text" name="suffix" value="{{ $official->suffix ?? '' }}" placeholder="Suffix"
                   class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
        </div>

        <!-- Gender & Birthday -->
        @php $gender = ucfirst(strtolower(trim($official->gender))); @endphp
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <input type="date" name="birthday" id="editBirthdate{{ $official->id }}" value="{{ $official->birthday->format('Y-m-d') }}" required
                   class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <select name="civil_status" required class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
                ...
                <option value="Widow" {{ $official->civil_status == 'Widow' ? 'selected' : '' }}>Widow</option>
                <option value="Separated" {{ $official->civil_status == 'Separated' ? 'selected' : '' }}>Separated</option>
            </select>

            <input type="text" name="phone" value="{{ $official->phone }}" placeholder="Phone Number" maxlength="11"
                   class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">

            <input type="email" name="email" value="{{ $official->email }}" placeholder="Email Address"
...
<div>
    ...
    <select name="position" id="editPosition{{ $official->id }}" required
            class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
        <option value="">Select Position</option>
        <option value="Kagawad" {{ $official->position == 'Kagawad' ? 'selected' : '' }}>Kagawad</option>
        <option value="Vaw Desk Officer" {{ $official->position == 'Vaw Desk Officer' ? 'selected' : '' }}>Vaw Desk Officer</option>
        <option value="Barangay Treasurer" {{ $official->position == 'Barangay Treasurer' ? 'selected' : '' }}>Barangay Treasurer</option>
...
<div>
    ...
    <select name="start_year" id="startYearDiv{{ $official->id }}"
            class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
        @for ($y = date('Y'); $y >= 2000; $y--)
            <option value="{{ $y }}" {{ $official->start_year == $y ? 'selected' : '' }}>{{ $y }}</option>
        @endfor
    </select>
</div>
...
<div>
    ...
    <select name="end_year" id="endYearDiv{{ $official->id }}"
            class="border rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
        @for ($y = date('Y') - 6; $y <= date('Y') + 6; $y++)
            <option value="{{ $y }}" {{ $official->end_year == $y ? 'selected' : '' }}>{{ $y }}</option>
        @endfor
    </select>
</div>
</div>
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-4">
                @php $savedCategories = $official->categories ?? []; @endphp
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="categories[]" value="PWD" {{ in_array('PWD', $savedCategories) ? 'checked' : '' }}>
                    PWD
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" id="editSenior{{ $official->id }}" name="categories[]" value="Senior" {{ in_array('Senior', $savedCategories) ? 'checked' : '' }}>
                    Senior
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="categories[]" value="Indigenous" {{ in_array('Indigenous', $savedCategories) ? 'checked' : '' }}>
                    Indigenous People
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="categories[]" value="Solo Parent" {{ in_array('Solo Parent', $savedCategories) ? 'checked' : '' }}>
                    Solo Parent
                </label>
            </div>
        </div>
...
const positionSelect = document.getElementById('editPosition{{ $official->id }}');
const startYearDiv = document.getElementById('startYearDiv{{ $official->id }}');
const endYearDiv = document.getElementById('endYearDiv{{ $official->id }}');
...
const birthInput = document.getElementById('editBirthdate{{ $official->id }}');
const seniorCheckbox = document.getElementById('editSenior{{ $official->id }}');
