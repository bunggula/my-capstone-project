<form action="{{ route('abc.announcements.update', $announcement->id) }}" 
    ...
    <div>
        <textarea name="details" rows="4" placeholder="Details"
                  class="border rounded-lg px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none" 
                  required>{{ old('details', $announcement->details) }}</textarea>
    </div>
...
<div>
    ...
    <select name="posted_by" 
            class="border rounded-lg px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
        <option value="abc_president" {{ $announcement->posted_by === 'abc_president' ? 'selected' : '' }}>
            ABC President
        </option>
        <option value="sk_president" {{ $announcement->posted_by === 'sk_president' ? 'selected' : '' }}>
            SK President
        </option>
    </select>
    ...
    {{-- Target Audience --}}
    <div x-data="{ target: '{{ old('target', $announcement->target) }}' }">
        ...
        <div x-show="target === 'all'" class="mt-2" x-transition>
            ...
            <select name="role_all"
                ...
                <option value="brgy_captain" {{ $announcement->target_role == 'brgy_captain' ? 'selected' : '' }}>Barangay Captain</option>
                <option value="secretary" {{ $announcement->target_role == 'secretary' ? 'selected' : '' }}>Secretary</option>
            </select>
        </div>
        ...
        <div x-show="target === 'specific'" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2" x-transition>
            <select name="barangay_id"
                ...
                @foreach ($barangays as $barangay)
                    <option value="{{ $barangay->id }}" {{ $announcement->barangay_id == $barangay->id ? 'selected' : '' }}>
                        {{ $barangay->name }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                @foreach ($announcement->images as $img)
                    <div class="relative group overflow-hidden rounded-lg shadow-md">
                        <img src="{{ asset('storage/' . $img->path) }}" class="w-full h-28 object-cover rounded">
