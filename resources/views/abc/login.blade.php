...
@section('title', 'Login - LAOVISTA Admin')
...
<div class="absolute w-80 h-80 bg-indigo-400/20 blur-3xl rounded-full bottom-20 right-10 animate-pulse-slow"></div>
    ...
    @if($municipality && $municipality->logo)
        <img src="{{ asset('storage/' . $municipality->logo) }}" alt="{{ $municipality->name }} Logo" class="h-24 w-auto">
    @else
        <img src="{{ asset('images/logo.png') }}" alt="Default Logo" class="h-24 w-auto">
    @endif
</div>
...
<p class="text-center text-sm text-gray-600 mb-6 animate-fade-in">
    ...
    <span class="font-semibold text-blue-800">
        ...
        <form method="POST" action="{{ route('login.post') }}" autocomplete="off" class="animate-fade-in">
            @csrf
    ...
    <svg id="buttonSpinner" class="hidden animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
            ...
            <div class="mt-4 text-center">
                <a href="{{ route('password.request') }}" class="text-blue-600 hover:underline hover:text-blue-800 transition duration-300">
