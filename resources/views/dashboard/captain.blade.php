...
<div  x-cloak x-data="{ profileOpen: false, loggingOut: false }" class="relative">
    ...
    >
        ...
        >
            ...
            <!-- Flash Messages -->
            @if (session('success'))
                <div class="mb-4 bg-green-100 text-green-800 p-4 rounded shadow animate-fade-in">✅ {{ session('success') }}</div>
            @endif
            @if (session('error'))
                <div class="mb-4 bg-red-100 text-red-800 p-4 rounded shadow animate-fade-in">❌ {{ session('error') }}</div>
            @endif
...
<!-- 📊 Charts Section -->
<div class="mt-8">
            @include('partials.captain_chart')
        </div>
