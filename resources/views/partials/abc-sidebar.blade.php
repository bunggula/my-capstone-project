...
<div style="width: 250px; background-color: #ffffff; padding: 30px 20px; display: flex; flex-direction: column; font-family: 'Segoe UI', sans-serif; height: 100vh; border-right: 1px solid #e5e7eb;">
    ...
    <div style="margin-bottom: 40px; text-align: center;">
        {{-- Logo Image --}}
     <img 
    src="{{ $municipality && $municipality->logo 
...
>
        ...
        {{-- Title --}}
        <a href="{{ route('abc.dashboard') }}" style="text-decoration: none; color: #1e3a8a; font-size: 22px; font-weight: 700;">
             ABC Admin
        </a>
    </div>
    ...
    <!-- Navigation Links -->
        <nav style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <a href="{{ route('abc.dashboard') }}" class="nav-link {{ request()->routeIs('abc.dashboard') ? 'active' : '' }}">
                <i data-lucide="home" class="icon"></i>
                <span>Dashboard</span>
            </a>
<div x-cloak x-data="{ 
        openPopulation: {{ request()->routeIs('abc.residents.*') || request()->routeIs('abc.all.*') ? 'true' : 'false' }},
    ...
    <div x-show="openPopulation" class="ml-4 mt-2 space-y-1">
        ...
        <a href="{{ route('abc.residents.index') }}"
           :class="[
               'nav-link flex items-center w-full whitespace-nowrap', 
               {{ request()->routeIs('abc.residents.*') ? "'active'" : "''" }}
           ]">
            <div class="flex items-center gap-2">
        ...
        <a href="{{ route('abc.all.index') }}"
           :class="[
               'nav-link flex items-center w-full whitespace-nowrap', 
               {{ request()->routeIs('abc.all.*') ? "'active'" : "''" }}
           ]">
            <div class="flex items-center gap-2">
                <i data-lucide="users" class="icon"></i>
...
<div class="relative">
    <a href="{{ route('abc.services.index') }}" 
       class="nav-link flex items-center justify-between w-full {{ request()->routeIs('abc.services.*') ? 'active' : '' }}">
...
<div 
   x-cloak x-data="{ open: {{ request()->routeIs('abc.announcements.*') || request()->routeIs('abc.events.*') ? 'true' : 'false' }} }" 
    class="nav-group"
>
    <!-- Parent Button -->
    <button 
        ...
        {{ request()->routeIs('abc.announcements.*') || request()->routeIs('abc.events.*') ? 'active' : '' }}">
    ...
    <div x-cloak x-show="open" x-collapse class="ml-6 mt-1 space-y-1">
        ...
        <a href="{{ route('abc.announcements.index') }}"
           class="nav-sublink nav-link {{ request()->routeIs('abc.announcements.*') ? 'active' : '' }}">
...
<div class="relative">
    <a href="{{ route('abc.events.index') }}" 
       class="nav-link nav-sublink flex items-center justify-between w-full text-left 
              ...
              {{ request()->routeIs('abc.events.*') ? 'active' : '' }}">
...
<div class="relative">
    <a href="{{ route('abc.concerns.index') }}" 
       class="nav-link flex items-center justify-between w-full {{ request()->routeIs('abc.concerns.*') ? 'active' : '' }}">
...
<!-- Proposals Button -->
<div class="relative">
    <a href="{{ route('abc.proposals.index') }}" 
       class="nav-link flex items-center justify-between w-full {{ request()->routeIs('abc.proposals.*') ? 'active' : '' }}">
...
<div class="relative">
    <a href="{{ route('abc.accounts.list') }}" 
       class="nav-link flex items-center justify-between w-full
              {{ request()->routeIs('abc.accounts.*') ? 'active' : '' }}">
        <div class="inline-flex items-center">
