...
<?php $__env->startSection('title', 'All Barangays'); ?>
...
<div class="bg-white rounded-xl p-6 shadow-md">
    <h2 class="text-xl font-semibold mb-4">List of Barangays (<?php echo e($barangays->count()); ?>)</h2>

    <!-- Municipality Section -->
    <div class="mb-6 flex items-center gap-4">
        <h2 class="text-xl font-semibold flex items-center gap-2">
            Municipality:
            <span id="municipalityName"><?php echo e(optional($barangays->first()->municipality)->name ?? 'N/A'); ?></span>
        </h2>

        <div class="flex items-center gap-2">
            <img id="municipalityMainLogo" 
                 src="<?php echo e(optional($barangays->first()->municipality)->logo 
                         ? asset('storage/' . $barangays->first()->municipality->logo) 
                         : asset('images/default_logo.png')); ?>" 
                 alt="Logo" 
                 class="w-10 h-10 object-cover rounded">

            <?php if($barangays->first()->municipality): ?>
                <button id="editMunicipalityBtn" 
                    data-id="<?php echo e($barangays->first()->municipality->id); ?>" 
                    data-url="/abc/municipality/<?php echo e($barangays->first()->municipality->id); ?>/update"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" id="barangay-grid">
                    <?php $__currentLoopData = $barangays; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $barangay): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="bg-white shadow-md rounded-xl p-4 hover:bg-blue-50 transition" 
                            data-id="<?php echo e($barangay->id); ?>" 
                            data-name="<?php echo e($barangay->name); ?>">
                            
                            <h3 class="text-lg font-semibold text-gray-800"><?php echo e($barangay->name); ?></h3>
                            <p class="text-sm text-green-600 font-medium mt-1">Residents: <?php echo e($barangay->approved_residents_count); ?></p>
        ...
        <form id="municipalityForm" class="px-6 py-4" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="municipalityInput" class="w-full border rounded px-3 py-2" required
                    value="<?php echo e($barangays->first()->municipality->name ?? ''); ?>">
            </div>
            <div class="mb-4">
                <label class="block mb-1">Municipality Logo</label>
                <img id="municipalityLogoPreview" 
                     src="<?php echo e($barangays->first()->municipality->logo ? asset('storage/' . $barangays->first()->municipality->logo) : ''); ?>" 
                     class="w-20 h-20 mb-2 object-cover rounded <?php echo e($barangays->first()->municipality->logo ? '' : 'hidden'); ?>">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    modalForm.addEventListener('submit', async (e) => {
        ...
        const url = isEdit ? `/abc/barangays/${id}` : `<?php echo e(route('abc.barangays.store')); ?>`;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Acer\OneDrive\Desktop\Laovista\laovista\resources\views/abc/barangays/index.blade.php ENDPATH**/ ?>