...
const residentData = <?php echo json_encode($residentsPerBarangay, 15, 512) ?>;
...
const residentsChart = new Chart(residentsCtx, {
    type: 'pie',
    data: {
        labels: <?php echo json_encode($barangayNames, 15, 512) ?>,
        datasets: [{
            data: residentData,
            backgroundColor: generateColors(<?php echo json_encode($barangayNames, 15, 512) ?>.length),
            borderColor: '#fff',
            borderWidth: 2
        }]
    },
...
new Chart(document.getElementById('announcementsChart'), {
    ...
    data: {
        ...
        datasets: [
            {
                label: 'All Barangays',
                data: <?php echo json_encode($announcementBreakdownPerMonth['All Barangays'], 15, 512) ?>,
                borderColor: '#36A2EB',
            ...
            },
            {
                ...
                data: <?php echo json_encode($announcementBreakdownPerMonth['Specific Barangay'], 15, 512) ?>,
                borderColor: '#4BC0C0',
            ...
            {
                label: 'Specific Role',
                data: <?php echo json_encode($announcementBreakdownPerMonth['Specific Role'], 15, 512) ?>,
                borderColor: '#FFCE56',
...
const eventBarangays = <?php echo json_encode($barangayNames, 15, 512) ?>;
const eventsPerBarangay = <?php echo json_encode($eventsPerBarangay, 15, 512) ?>;
...
new Chart(document.getElementById('proposalsChart'), {
    ...
    data: {
        ...
        datasets: [
            { 
                label: 'Pending', 
                data: <?php echo json_encode($proposalsPerMonth['pending'], 15, 512) ?>, 
            ...
            { 
                label: 'Approved', 
                data: <?php echo json_encode($proposalsPerMonth['approved'], 15, 512) ?>, 
            ...
            { 
                label: 'Rejected', 
                data: <?php echo json_encode($proposalsPerMonth['rejected'], 15, 512) ?>, 
...
const documentTypes = <?php echo json_encode($documentTypes, 15, 512) ?>;
const months = <?php echo json_encode($months, 15, 512) ?>;
const detailedData = <?php echo json_encode($servicesPerDocumentTypeDetailed, 15, 512) ?>;
...
/* ========================= RESOLVED CASES CHART ========================= */
const resolvedBarangays = <?php echo json_encode($barangayNames, 15, 512) ?>;
const resolvedCountsPerMonth = <?php echo json_encode($resolvedDataPerMonth['resolvedCounts'], 15, 512) ?>;
...
</script><?php /**PATH C:\Users\Acer\OneDrive\Desktop\Laovista\laovista\resources\views/partials/abc_chart.blade.php ENDPATH**/ ?>