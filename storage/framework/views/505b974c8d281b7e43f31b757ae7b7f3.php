...
<?php $__env->startSection('title', 'ABC Dashboard'); ?>
    ...
    <main class="flex-1 relative overflow-hidden">
        ...
        <div class="relative z-20 overflow-y-auto p-8" style="height: calc(100vh);">
            ...
            <div class="max-w-7xl mx-auto flex justify-between items-center mb-10">
                <h2 class="text-3xl font-bold text-black"></h2>

                <div class="flex items-center space-x-6 relative">

                   <!-- Notifications Dropdown -->
   <!-- Notifications Dropdown -->
<!-- 🔔 Notifications + 👤 Profile (Unified Alpine Component) -->
<div 
    x-data='{
        notifOpen: false,
        profileOpen: false,
        ...
        toggleNotif() {
            ...
            if (this.notifOpen) this.profileOpen = false; // isara profile kapag nag-open notif
            if (this.notifOpen) this.markAllRead();
        },
        toggleProfile() {
            this.profileOpen = !this.profileOpen;
            if (this.profileOpen) this.notifOpen = false; // isara notif kapag nag-open profile
        },
        loadMore() { this.visible += 5 },
        markRead(key) {
            let notif = this.notifications.find(n => n.key === key);
            if (notif) notif.read = true;
            fetch("/abc/notifications/mark-read", {
                method: "POST",
                headers: {
    ...
    <div class="relative">
        ...
        >
            ...
            <div class="px-4 py-3 border-b flex justify-between items-center">
                ...
                <button @click="notifOpen = false" class="text-gray-400 hover:text-gray-600">
            ...
            <div class="px-4 py-3 text-right border-t">
                <button @click="notifOpen = false" class="text-indigo-600 text-sm font-medium hover:underline">
                    Close
                </button>
            </div>
        </div>
...
<div x-data="{ profileOpen: false, loggingOut: false }" class="relative">
    ...
    >
        ...
        <a href="<?php echo e(route('settings.profile')); ?>" class="block px-4 py-2 hover:bg-gray-100">
            Edit Profile
        </a>
        <a href="<?php echo e(route('settings.password')); ?>" class="block px-4 py-2 hover:bg-gray-100">
            Change Password
        </a>
...
<div class="max-w-7xl mx-auto px-4 mt-6 grid grid-cols-1 lg:grid-cols-4 gap-6 ">
    ...
    <div class="lg:col-span-3 space-y-8">
         <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <?php
                        $cards = [
                            ['title'=>'Residents','description'=>"View Approved residents' data.",'icon'=>'🧑‍🤝‍🧑','badge'=>$totalApprovedResidents,'badgeColor'=>'green','route'=>route('residents.index')],
                            ['title'=>'Announcements','description'=>"View, create, or manage official announcements.",'icon'=>'📢','badge'=>$totalAnnouncements,'badgeColor'=>'yellow','route'=>route('abc.announcements.index')],
                            ['title'=>'Barangay Events','description'=>"See all events happening in barangays.",'icon'=>'🎉','badge'=>$totalApprovedEvents,'badgeColor'=>'purple','route'=>route('abc.events.index')],
                            ['title'=>'Barangays','description'=>"View all registered barangays details.",'icon'=>'🏘️','badge'=>$totalBarangays,'badgeColor'=>'red','route'=>route('abc.barangays.index')],
                            ['title'=>'Proposals','description'=>"Review and manage pending or approved barangay proposals.",'icon'=>'📥','badge'=>$totalApprovedProposals,'badgeColor'=>'cyan','route'=>route('abc.proposals.index')],
                            ['title'=>'Services','description'=>"Access and track completed municipal document requests.",'icon'=>'📄','badge'=>$totalCompletedServices,'badgeColor'=>'indigo','route'=>'#'],
                        ];
                    ?>
                        ...
                        <a href="<?php echo e($card['route']); ?>"
                           class="bg-white rounded-2xl shadow-xl p-6 border border-transparent hover:bg-blue-50 hover:shadow-2xl hover:border-blue-300 transition transform hover:scale-[1.03] duration-300 relative flex flex-col justify-between h-full animate-fade-up">
                            <?php if(isset($card['badge'])): ?>
                                <div class="absolute top-4 right-4 bg-<?php echo e($card['badgeColor']); ?>-100 text-<?php echo e($card['badgeColor']); ?>-700 text-sm font-bold px-3 py-1 rounded-full shadow animate-badge">
                            ...
                            <div class="flex items-center gap-4 mb-4">
                                ...
                                <h3 class="text-xl font-bold text-black"><?php echo e($card['title']); ?></h3>
                            </div>
                            <p class="text-sm text-black"><?php echo e($card['description']); ?></p>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
    </div>

<!-- 📅 RIGHT SIDE: Upcoming Announcements & Events -->
<!-- 📅 RIGHT SIDE: Upcoming Announcements & Events (Ultra Compact) -->
<div class="lg:col-span-1">
    <div class="bg-white rounded-lg shadow-md p-2 compact-upcoming h-full overflow-y-auto">
        ...
        <div class="bg-blue-50 rounded-md p-2 shadow-inner text-center mb-2">
            ...
            <p class="text-xs font-bold text-gray-900">
                <?php echo e(\Carbon\Carbon::now()->format('l, M d, Y')); ?>

            </p>
        </div>

        <h3 class="text-xs font-semibold text-gray-700 mb-2 text-center">
        ...
        <div class="mb-2">
            ...
            <?php if($upcomingAnnouncements->count()): ?>
                <?php
                ...
                <ul class="space-y-0.5 max-h-24 overflow-y-auto pr-1">
                    <?php $__currentLoopData = $upcomingAnnouncements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $announcement): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $isNext = $announcement->id === $nextAnnouncement?->id; ?>
                        <li class="border-b pb-0.5 hover:translate-x-0.5 transition-transform duration-200 
                                   <?php echo e($isNext ? 'bg-yellow-100 rounded p-0.5 animate-bounce-slow' : ''); ?>">
                            <h5 class="text-xs font-bold text-gray-900 truncate"><?php echo e($announcement->title); ?></h5>
                            <p class="text-xs text-gray-600">
                                <?php echo e(\Carbon\Carbon::parse($announcement->date)->format('M d, Y')); ?>
            ...
            <?php if($upcomingEvents->count()): ?>
                ...
                <ul class="space-y-0.5 max-h-24 overflow-y-auto pr-1">
                    <?php $__currentLoopData = $upcomingEvents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $isNextEvent = $event->id === $nextEvent?->id; ?>
                        <li class="border-b pb-0.5 hover:translate-x-0.5 transition-transform duration-200 
                                   <?php echo e($isNextEvent ? 'bg-green-100 rounded p-0.5 animate-bounce-slow' : ''); ?>">
                            <h5 class="text-xs font-bold text-gray-900 truncate"><?php echo e($event->title); ?></h5>
                            <p class="text-xs text-gray-600">
                                <?php echo e(\Carbon\Carbon::parse($event->date)->format('M d, Y')); ?>

                                @ <?php echo e(\Carbon\Carbon::parse($event->time)->format('g:i A')); ?>
 ...
 <div class="mt-8">
            <?php echo $__env->make('partials.abc_chart', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
...
.animate-fade-up { animation: fade-up 0.8s ease-out forwards; }
    ...
    /* Upcoming list hover effect */
    .upcoming-item:hover { transform: translateX(5px); transition: transform 0.3s ease-in-out; }

    @keyframes bell-shake { 0%,100% { transform: rotate(0deg); } 20% { transform: rotate(-15deg); } 40% { transform: rotate(15deg); } 60% { transform: rotate(-10deg); } 80% { transform: rotate(10deg); } }
    .animate-bell-shake { animation: bell-shake 3s ease-in-out infinite; }

</style>

<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Acer\OneDrive\Desktop\Laovista\laovista\resources\views/abc/dashboard.blade.php ENDPATH**/ ?>