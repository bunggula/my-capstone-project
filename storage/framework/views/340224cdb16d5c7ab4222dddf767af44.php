...
<div 
    x-data='{
        ...
        markRead(key) {
            ...
            fetch("/captain/notifications/mark-read", {
                method: "POST",
                headers: {
        ...
        markAllRead() {
            ...
            fetch("/captain/notifications/mark-all-read", {
                method: "POST",
...
>
    <!-- Bell Button -->
    <button 
        @click="notifOpen = !notifOpen; markAllRead()" 
        ...
        <div class="px-4 py-3 text-right border-t">
            <button @click="notifOpen = false" class="text-indigo-600 text-sm font-medium hover:underline">
                Close
                     ...
                     <!-- Profile Dropdown -->
<div  x-cloak x-data="{ profileOpen: false, loggingOut: false }" class="relative">
    ...
    >
        ...
        >
            ...
            <?php if(session('success')): ?>
                <div class="mb-4 bg-green-100 text-green-800 p-4 rounded shadow animate-fade-in">✅ <?php echo e(session('success')); ?></div>
            <?php endif; ?>
            <?php if(session('error')): ?>
                <div class="mb-4 bg-red-100 text-red-800 p-4 rounded shadow animate-fade-in">❌ <?php echo e(session('error')); ?></div>
            <?php endif; ?>
    ...
    <div class="lg:col-span-3 space-y-8">
       <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <?php
                $cards = [
                    ['title'=>'Residents','description'=>'All registered residents.','icon'=>'🧑‍🤝‍🧑','badge'=>$approvedResidentsCount,'badgeColor'=>'blue','route'=>route('captain.residents.index')],
                    ['title'=>'Events','description'=>'Review and approve events.','icon'=>'🎉','badge'=>$approvedEventsCount,'badgeColor'=>'green','route'=>route('captain.events.index')],
                    ['title'=>'Announcements','description'=>'Check official announcements.','icon'=>'📢','badge'=>$announcementsCount,'badgeColor'=>'yellow','route'=>route('captain.announcements.index')],
                    ['title'=>'Concerns','description'=>'Total Resolved.','icon'=>'❗','badge'=>$concernsCount,'badgeColor'=>'red','route'=>route('captain.concerns.index')],
                    
                    ['title'=>'Services','description'=>'View services.','icon'=>'📝️','badge'=>$documentRequestCount,'badgeColor'=>'purple','route'=>route('captain.documents.index')],
                    ['title'=>'Proposals','description'=>'Total submitted proposals.','icon'=>'📄','badge'=>$totalProposals,'badgeColor'=>'teal','route'=>route('captain.proposal.index')],
                ];
            ?>
...
<!-- 📊 Charts Section -->
<div class="mt-8">
            <?php echo $__env->make('partials.captain_chart', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Acer\OneDrive\Desktop\Laovista\laovista\resources\views/dashboard/captain.blade.php ENDPATH**/ ?>