...
<?php $__env->startSection('title', 'Municipality Barangay Officials'); ?>
...
<a href="<?php echo e(route('abc.accounts.list', array_merge(request()->except('status'), ['status' => 'active']))); ?>"
   class="block p-4 rounded shadow border-l-4 
          <?php echo e((request('status') === 'active' || !request('status')) 
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-2xl font-bold"><?php echo e($statusCounts['active'] ?? 0); ?></p>
        </div>
...
<a href="<?php echo e(route('abc.accounts.list', array_merge(request()->except('status'), ['status' => 'inactive']))); ?>"
   class="block p-4 rounded shadow border-l-4 
          <?php echo e(request('status') === 'inactive' ? 'bg-red-200 border-red-600 text-red-900' : 'bg-red-100 border-red-500 text-red-700'); ?>">
    <div class="flex items-center justify-between">
        <div>
            <h4 class="font-semibold text-sm">
                Inactive Accounts
            ...
            <p class="text-2xl font-bold"><?php echo e($statusCounts['inactive'] ?? 0); ?></p>
        </div>
        <svg class="h-8 w-8 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <table class="min-w-full text-sm border border-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="<?php echo e($loop->even ? 'bg-gray-100' : 'bg-white'); ?> hover:bg-gray-200">
                                <td class="px-4 py-2"><?php echo e($users->firstItem() + $loop->index); ?></td>
                                <td class="px-4 py-2">
                                   
                                    <?php echo e($user->first_name); ?>

                                    <?php echo e($user->middle_name ? strtoupper(substr($user->middle_name, 0, 1)) . '.' : ''); ?>

                                    <?php echo e($user->last_name); ?>

                                    <?php echo e($user->suffix ?? ''); ?>

                                </td>
                              
                                <td class="px-4 py-2 capitalize"><?php echo e(str_replace('_', ' ', $user->role)); ?></td>
                                <td class="px-4 py-2"><?php echo e($user->barangay->name ?? 'N/A'); ?></td>
...
<button 
    onclick="loadViewModal(<?php echo e($user->id); ?>)" 
    title="View"
...
<?php if($user->role === 'brgy_captain'): ?>
<button 
    onclick="loadEditModal(<?php echo e($user->id); ?>)" 
    title="Edit"
...
<?php if($user->role === 'brgy_captain'): ?>
<form id="toggle-form-<?php echo e($user->id); ?>" action="<?php echo e(route('abc.toggle_status', $user->id)); ?>" method="POST" class="inline">
    <?php echo csrf_field(); ?>
    <?php echo method_field('PATCH'); ?>
    <button 
        type="button"
        onclick="confirmToggle(<?php echo e($user->id); ?>, '<?php echo e($user->status); ?>', '<?php echo e($user->first_name); ?> <?php echo e($user->last_name); ?>', '<?php echo e($user->barangay->name ?? 'N/A'); ?>')"
        title="<?php echo e($user->status === 'active' ? 'Deactivate' : 'Activate'); ?>"
        class="inline-flex items-center justify-center w-8 h-8 rounded-md <?php echo e($user->status === 'active' ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-green-100 text-green-600 hover:bg-green-200'); ?> transition"
    >
...
<div id="addAccountModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden justify-center items-center">
    <div class="bg-white w-full max-w-3xl p-6 rounded-lg relative shadow-lg overflow-y-auto max-h-[90vh]">
        ...
        <div id="addAccountContent" class="text-sm">
            ...
            <?php echo $__env->make('abc.add_account', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?> 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Acer\OneDrive\Desktop\Laovista\laovista\resources\views/abc/list_accounts.blade.php ENDPATH**/ ?>