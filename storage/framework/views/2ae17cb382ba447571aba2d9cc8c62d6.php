...
<div x-data="{
    ...
    <form action="<?php echo e(route('captain.barangay-info.update')); ?>" method="POST" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="<?php echo e(old('name', $barangay->name)); ?>" class="w-full border px-3 py-2 rounded bg-gray-100" readonly>
            </div>

            <!-- Chairperson -->
            <div>
                <label class="font-semibold block mb-1">Chairperson Name</label>
                <input type="text" name="chairperson" value="<?php echo e(old('chairperson', $barangay->chairperson ?? ($chairperson->full_name ?? ''))); ?>" class="w-full border px-3 py-2 rounded bg-gray-100" readonly>
            </div>

            <!-- Contact Number -->
            <div>
                <label class="font-semibold block mb-1">Contact Number</label>
                <input type="text" name="contact_number" value="<?php echo e(old('contact_number', $barangay->contact_number)); ?>" class="w-full border px-3 py-2 rounded">
            </div>

            <!-- Email -->
            <div>
                <label class="font-semibold block mb-1">Email Address</label>
                <input type="email" name="email" value="<?php echo e(old('email', auth()->user()->email)); ?>" class="w-full border px-3 py-2 rounded bg-gray-100" readonly>
            </div>

            <!-- Office Hours -->
            <div>
                <label class="font-semibold block mb-1">Office Hours</label>
                <input type="text" name="office_hours" value="<?php echo e(old('office_hours', $barangay->office_hours)); ?>" class="w-full border px-3 py-2 rounded">
            </div>
        </div>
    ...
    <div class="mt-10">
        <div class="flex justify-between items-center mb-4">
            ...
            <?php if (! ($secretary && $secretary->status === 'active')): ?>
        ...
        <?php if($secretary): ?>
            <div class="bg-white shadow rounded p-4 flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    <div>
                        <p class="font-semibold"><?php echo e($secretary->full_name); ?></p>
                        <p class="text-sm text-gray-500"><?php echo e($secretary->email); ?></p>
                    </div>
                    <div>
                        <?php if($secretary->status === 'active'): ?>
                ...
                <div class="flex items-center space-x-2">
                    <!-- View Button -->
                    <button @click="selectedSecretary = <?php echo e($secretary->id); ?>; viewSecretaryModal = true" title="View Details"  class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-blue-700 text-white hover:bg-blue-600 transition">
                    ...
                    <button @click="
                        editSecretaryData = {
                            id: <?php echo e($secretary->id); ?>,
                            first_name: '<?php echo e(addslashes($secretary->first_name)); ?>',
                            middle_name: '<?php echo e(addslashes($secretary->middle_name ?? '')); ?>',
                            last_name: '<?php echo e(addslashes($secretary->last_name)); ?>',
                            email: '<?php echo e(addslashes($secretary->email)); ?>',
                            gender: '<?php echo e($secretary->gender); ?>',
                            birthday: '<?php echo e($secretary->birthday); ?>'
                        };
        ...
        <path stroke-linecap="round" stroke-linejoin="round"
              d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5M18.5 2.5a2.121 2.121 0 113 3L12 15l-4 1 1-4 9.5-9.5z" />
                    ...
                    <form id="toggle-form-<?php echo e($secretary->id); ?>" action="<?php echo e(route('captain.secretary.toggle', $secretary->id)); ?>" method="POST" class="inline">
    <?php echo csrf_field(); ?>
    <?php echo method_field('PATCH'); ?>
    <button 
        type="button"
        onclick="event.stopPropagation(); confirmToggle(<?php echo e($secretary->id); ?>, '<?php echo e($secretary->status); ?>', '<?php echo e($secretary->first_name); ?> <?php echo e($secretary->last_name); ?>')"
        title="<?php echo e($secretary->status === 'active' ? 'Deactivate Secretary' : 'Activate Secretary'); ?>"
        class="inline-flex items-center justify-center w-8 h-8 rounded-md <?php echo e($secretary->status === 'active' ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-green-100 text-green-600 hover:bg-green-200'); ?> transition"
    >
    ...
    <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md transform transition-all duration-300 scale-95 x-show.transition.scale.100">
        ...
        <div class="space-y-3">
            <div class="flex items-center">
                ...
                <span class="text-gray-800"><?php echo e($secretary->full_name ?? '-'); ?></span>
            </div>
            <div class="flex items-center">
                <span class="w-32 font-medium text-gray-600">Email:</span>
                <span class="text-gray-800"><?php echo e($secretary->email ?? '-'); ?></span>
            </div>
            <div class="flex items-center">
                <span class="w-32 font-medium text-gray-600">Gender:</span>
                <span class="text-gray-800"><?php echo e($secretary->gender ?? '-'); ?></span>
            </div>
            <?php
    use Carbon\Carbon;
...
<div class="flex items-center">
    ...
    <span class="text-gray-800"><?php echo e($birthday); ?></span>
</div>

            <div class="flex items-center">
                <span class="w-32 font-medium text-gray-600">Status:</span>
                <span class="px-2 py-1 rounded-full text-sm font-semibold <?php echo e($secretary->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?>">
                    <?php echo e(ucfirst($secretary->status ?? '-')); ?>
...
<?php /**PATH C:\Users\Acer\OneDrive\Desktop\Laovista\laovista\resources\views/captain/barangay-info/edit.blade.php ENDPATH**/ ?>